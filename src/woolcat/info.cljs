  ...
  [:div.col-span-2
   [:div {:style {:display "grid", :grid-template-columns "auto 1fr", :gap "32px"}}
    ...
    [:div {:style {:max-width "600px" :justify-self "start" :align-self "start"}}
     ...
     [:p "Chan Ann Chuang is an architect by training, originally from China and now based in Finland. Her journey from architecture to interior design and handicrafts has been one of exploration and rediscovery. After years of contributing as an architect to large-scale projects, Ann realized that her passion lies in creating on a more intimate scale — crafting pieces where she can design and construct every detail with her full control."]
