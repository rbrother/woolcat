...
(defglobal
  ...
  [:div.hover-image-container {:position "relative" :width "100%" :height "100%"}]
  [:img.hover-image-main {:width "100%" :height "100%" :object-fit "cover" :position "absolute" :top "0" :left "0" :z-index "2" :transition "opacity 0.3s ease"}]
  [:img.hover-image-secondary {:width "100%" :height "100%" :object-fit "cover" :position "absolute" :top "0" :left "0" :z-index "1"}]
  [:div.hover-text-overlay {:position "absolute" :top "50%" :left "50%" :transform "translate(-50%, -50%)"
                            :z-index "3" :color "black" :font-size "24px"
                            :opacity "0" :transition "opacity 0.3s ease" :pointer-events "none"}]
  [:div.hover-image-container:hover [:img.hover-image-main {:opacity "0"}]]
  [:div.hover-image-container:hover [:div.hover-text-overlay {:opacity "1"}]]
